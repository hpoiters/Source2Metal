"""Publish immutable Source2Metal v3.3.1 as Latest after CI succeeds."""
import hashlib, json, os, subprocess
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
REPO="hpoiters/Source2Metal"; VERSION="3.3.1"; TAG="v3.3.1"
...
def main():
    ...
    assert all(t["name"]!=TAG for t in api("tags?per_page=100")),"v3.3.1 already exists; refusing replacement"
    ...
    subprocess.run(["gh","release","create",TAG,*map(str,files),"--repo",REPO,"--target",sha,
                    "--title","Source2Metal v3.3.1 — PGN warning no longer pauses processing",
