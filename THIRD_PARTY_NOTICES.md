...
SyzygyCheck v2.0.7 embeds an unchanged `tbcheck.exe` and extracts/runs it as a separate checksum engine. `tbcheck` originates from Ronald de Man's Syzygy tablebase generator project:
...
`tbcheck.exe` is an upstream open-source binary and is not claimed as original Source2Metal code. It should not be separately signed using a Source2Metal project certificate unless the signing provider explicitly permits that treatment. SignPath Foundation policy allows unsigned upstream OSS binaries to be included in signed packages.
