...
// Progress reports coarse converter progress without changing the BIN2PGN
// conversion semantics. Overall is a deliberately approximate 0..1 estimate
// used only for a user-facing ETA; Current/Total remain the factual counters.
type Progress struct {
	Phase              string
	Current            int64
	Total              int64
	ReachablePositions int64
	ReachableRecords   int64
	Overall            float64
}

type ProgressFunc func(Progress)

func emitProgress(fn ProgressFunc, phase string, current, total, positions, records int64) {
	if fn == nil {
		return
	}
	fn(Progress{
		...
		ReachableRecords:   records,
		Overall:            overallProgress(phase, current, total),
	})
}

func overallProgress(phase string, current, total int64) float64 {
	fraction := 0.0
	if total > 0 {
	...
	var start, span float64
	switch phase {
	case "index":
		start, span = 0.00, 0.08
	case "normalize":
		start, span = 0.08, 0.04
	case "graph":
		start, span = 0.12, 0.60
	case "lines":
		start, span = 0.72, 0.18
	case "write":
		start, span = 0.90, 0.05
	case "validate":
		return 0.95
	case "coverage":
		start, span = 0.98, 0.015
	case "done":
		return 1
	default:
		return 0
...
func progressDue(current, total, step int64) bool {
	return current == 0 || current == total || (step > 0 && current%step == 0)
}

// ConvertWithProgress is Convert with a read-only progress callback. It uses the
// same validation, line construction and final coverage checks as Convert.
func ConvertWithProgress(input, output string, maxPly int, fn ProgressFunc) (Stats, error) {
	if maxPly < 1 || maxPly > 120 {
		return Stats{}, fmt.Errorf("BIN depth must be between 1 and 120 ply")
	}
	return convertBookWithProgress(input, output, maxPly, fn)
}

func loadBookWithProgress(path string, fn ProgressFunc) (Book, error) {
	f, err := os.Open(path)
	if err != nil {
		return Book{}, err
	}
...
func convertBookWithProgress(path, outPath string, maxPly int, fn ProgressFunc) (Stats, error) {
	book, err := loadBookWithProgress(path, fn)
	if err != nil {
		return Stats{}, err
	}
	stats := Stats{
	...
	g, err := buildReachableGraphWithProgress(book, maxPly, &stats, fn)
	if err != nil {
		return stats, err
	}
	lines, err := buildCompleteLinesWithProgress(g, maxPly, &stats, fn)
	if err != nil {
		return stats, err
	}
	if len(lines) == 0 {
...
func buildReachableGraphWithProgress(book Book, maxPly int, stats *Stats, fn ProgressFunc) (*BookGraph, error) {
	start := startBoard()
...
func buildCompleteLinesWithProgress(g *BookGraph, maxPly int, stats *Stats, fn ProgressFunc) ([]Line, error) {
