...
func reportTexts(lang string) reportText {
	...
	case "nl":
		return reportText{
			...
			language: "Taal", folder: "Controlemap", started: "Gestart", finished: "Beëindigd", duration: "Duur van deze run",
			...
			recoveryReference: "Voor herstel en betrouwbare downloadbronnen: zie %s.",
	...
	case "de":
		return reportText{
			...
			recoveryReference: "Hinweise zur Wiederherstellung und verlässliche Downloadquellen: siehe %s.",
	...
	case "fr":
		return reportText{
			...
			recoveryReference: "Pour la récupération et des sources de téléchargement fiables : voir %s.",
	...
	case "es":
		return reportText{
			...
			recoveryReference: "Para recuperación y fuentes de descarga fiables: consulte %s.",
	...
	case "zh":
		return reportText{
			...
			recoveryReference: "有关恢复方法和可靠下载来源，请查看 %s。",
	...
	case "ru":
		return reportText{
			...
			recoveryReference: "Для восстановления и надёжных источников загрузки см. %s.",
	...
	default:
		return reportText{
			...
			recoveryReference: "For recovery steps and reliable download sources, see %s.",
...
const releaseProgramFolder = "Program_AL"

func recoveryGuideReleasePath(lang string) string {
	// SyzygyCheck is a Windows release. Keep the displayed package path stable
	// and directly usable even when source tests run on another platform.
	return releaseProgramFolder + "\\" + recoveryGuideFilename(lang)
}
...
func fullResultPath(controlDir, name string) string {
	p := name
	if !filepath.IsAbs(p) {
		p = filepath.Join(controlDir, p)
	}
...
func writeReportV6(lang, dir, reportFolder string, files []item, totalBytes int64, scan realScanResult, testResult intentionalTestResult) (string, error) {
	t := reportTexts(lang)
	...
	if len(scan.Errors) > 0 {
		...
		sort.Strings(names)
		for _, n := range names {
			fmt.Fprintf(&b, "- %s: %s\r\n", fullResultPath(dir, n), scan.Errors[n])
		}
	}
	...
	data := []byte(b.String())
	if reportFolder != "" {
		if p, err := writeReportData(reportFolder, scan.Ended, data); err == nil {
			return p, nil
		}
	}
	// A chosen folder can become unavailable between selection and writing.
	// Never lose the report: fall back to the control folder beside the EXE.
	return writeReportData(filepath.Join(dir, reportFolderName(lang)), scan.Ended, data)
}

func writeSelfTestFailureReportV6(lang, dir, reportFolder, helperOrigin string, testResult intentionalTestResult) (string, error) {
	now := time.Now()
	scan := realScanResult{
	...
	return writeReportV6(lang, dir, reportFolder, nil, 0, scan, testResult)
}
