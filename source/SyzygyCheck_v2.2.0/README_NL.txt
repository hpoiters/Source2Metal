SyzygyCheck v2.2.0 — Alle talen
================================

Doel
----
De map waarin !SyzygyCheck_v2.2.0.exe staat heet in deze documentatie de Controlemap. SyzygyCheck controleert de
integriteit van Syzygy .rtbw/.rtbz-files vanaf die locatie. Voor iedere run kiest u:

  1 = Alleen deze Controlemap [standaard]
  2 = Deze Controlemap en alle submappen

Er wordt nooit in hogere mappen gezocht. Bij optie 2 worden echte submappen tot iedere diepte meegenomen.
Symbolische links en Windows-koppelingen worden niet gevolgd wanneer die buiten de Controlemap kunnen leiden.
Als de Controlemap de hoofdmap van een schijf is, worden de Windows-systeemmappen `$RECYCLE.BIN` en
`System Volume Information` overgeslagen. Verwijderde of beschermde files daarin horen niet bij de actieve tablebaseset.

Releasepakket
-------------
De volledige gebruikersrelease is één ZIP met vier vaste onderdelen in de hoofdmap:
...
Houd deze onderdelen bij elkaar. `1_READ_FIRST_AL.txt` geeft in alle zeven talen de korte startuitleg en verwijst
naar `Docs_AL\README_AL.txt`. De map Program_AL bevat de zeven herstelhandleidingen waarnaar een FAIL/ERROR-
rapport met een exact pad verwijst. De source zit niet in het gebruikerspakket. De aanbevolen broncodedownload is
de duidelijk benoemde release-asset `SyzygyCheck_v2.2.0_SOURCE.zip`; de repository staat op
...
Na voltooiing staat ook precies één witregel tussen het laatste percentage/resttijd-blok en de vraag waar het
resultaatrapport moet worden opgeslagen. Daardoor blijven twee inhoudelijk verschillende alinea's visueel gescheiden.
...
Resultaatrapport
----------------
Vlak voor het opslaan vraagt SyzygyCheck waar het gebruikersrapport moet komen:
1. op het Windows-bureaublad (Desktop) [standaard bij het eerste gebruik];
2. in de Controlemap (de map waarin !SyzygyCheck_v2.2.0.exe staat);
3. in een vrij te kiezen map via het Windows Verkenner-mapvenster.

Binnen de gekozen locatie maakt SyzygyCheck automatisch de verzamelmap CheckResult. Alle opeenvolgende
rapporten komen daarin terecht, zodat controles van een grote database over meerdere mappen niet meer voor losse
resultaatbestanden op het bureaublad zorgen. Chinees en Russisch gebruiken voor deze map een vertaalde naam in
hun eigen schrift. De laatst gekozen bestemming wordt onthouden en staat bij de volgende run als standaard;
Enter gebruikt die bestemming dan direct. Bij een vrije mapkeuze wordt het exacte pad onthouden. Als die map
niet meer bestaat of niet bereikbaar is, toont SyzygyCheck een waarschuwing met het onbereikbare vorige pad en
de consequentie dat Desktop nu de standaard is. De waarschuwing staat als afzonderlijke visuele alinea vóór het
bestemmingsmenu. Wie daarna op Enter drukt, slaat op in de verzamelmap op Desktop en vervangt daarmee de ongeldige
onthouden voorkeur. Het eindscherm toont altijd
het volledige opgeslagen pad. Normale OK-bestanden worden niet afzonderlijk opgesomd. Alleen samenvatting,
echte checksum-FAILs en operationele ERRORs worden vermeld.

Ook deze keuzeschermen zijn vensterschuifresistent: regels en lange onthouden paden worden aan de actuele
consolebreedte aangepast. Bij de breedteberekening tellen onder meer Chinese tekens volgens hun werkelijke
consolebreedte mee.

Iedere FAIL en ERROR wordt met het volledige Windows-pad vanaf de driveletter vermeld. Daardoor blijft bij een
check over meerdere submappen onmiddellijk zichtbaar waar de betreffende file staat.
...
---------------------
Bij een echte checksum-FAIL of operationele ERROR verwijst het resultaatrapport naar de meegeleverde
`Program_AL\SYZYGY_HERSTEL_NL.txt`. De release bevat in Program_AL dezelfde herstelhandleiding in alle zeven
talen. Daarin staan een korte
...
Als exact die helper naast SyzygyCheck staat, wordt hij gebruikt. Anders haalt v2.2.0 tijdens de run tijdelijk
de vaste v3.0.7-kopie op, controleert de SHA-256 en verwijdert de tijdelijke helper na afloop. Voor de gebruiker
blijft het controleprogramma zelf één EXE; de gebruikersrelease bevat daarnaast de starttekst, documentatie en
herstelhandleidingen die nodig zijn om alle scherm- en rapportverwijzingen werkelijk te kunnen volgen.

Praktijkvalidatie / status
--------------------------
v2.2.0 bouwt voort op v2.1.0 en de succesvol geteste v2.0.9 V9F. De verificatiekern en threadwerking zijn niet
gewijzigd. Nieuw zijn de expliciete keuze voor submappen, de harde grens bij de Controlemap en volledige foutpaden.
V9F blijft als eerste praktisch gevalideerde release onaangeroerd beschikbaar.
