...
SyzygyCheck v%s source can be extracted separately via Utilities > Extract
...
Der Quellcode von SyzygyCheck v%s kann separat über Hilfsprogramme >
...
De bron van SyzygyCheck v%s kan afzonderlijk worden uitgepakt via
...
Le source de SyzygyCheck v%s peut être extrait séparément via Utilitaires >
...
El código fuente de SyzygyCheck v%s puede extraerse por separado mediante
...
SyzygyCheck v%s 的源代码可通过“实用工具 > 解压 SyzygyCheck 源代码包（ZIP）”
...
Исходный код SyzygyCheck v%s можно отдельно извлечь через «Утилиты >
Извлечь пакет исходного кода SyzygyCheck (ZIP)» или с помощью
-extract-syzygy-source.
`)
	return fmt.Sprintf(tpl, version, buildDateUTC, len(embeddedSourceZip), embeddedSourceSHA256(), version, syzygyCheckDisplay)
}
