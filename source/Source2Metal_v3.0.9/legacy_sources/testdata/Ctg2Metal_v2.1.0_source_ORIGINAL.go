...
const learningInstructions = `Ctg2Metal v2.1.0 - INSTRUCTIE: Metal.pgn INLEREN IN FRITZ
================================================================

WAARVOOR IS DIT BESTAND?
------------------------
Metal.pgn is door Ctg2Metal speciaal gemaakt om het bijbehorende CTG-boek in
Fritz opnieuw te laten leren/tunen. Het is geen gewone partijendatabase.

BELANGRIJKSTE REGEL
-------------------
Gebruik bij "Leren van database" / "Bijleren uit database" GEEN spelersfilter.
    ...
    Speler               UIT   <-- NIET aanvinken
    Spelernaam           LEEG

    Partijen             ALLE partijen
                         dus van partij 1 t/m de laatste partij
...
Kort weergegeven:
    ...
    Speler          [ ]  naamveld leeg laten

    Partijen:       1 t/m laatste partij

WAAROM STAAT "SPELER" UIT?
---------------------------
Metal.pgn bevat uitsluitend leerpartijen die Ctg2Metal voor dit doel heeft
opgebouwd. Er hoeven dus geen gewone partijen met een spelersfilter te worden
uitgesloten.

Bovendien kan de spelernaamselectie in Fritz hoofdlettergevoelig zijn. Door
"Speler" niet aan te vinken en het naamveld leeg te laten, vermijden we dat
probleem volledig.

De PGN zelf gebruikt wel consequent de nette naam:

    [White "Metal"]
    [Black "Metal"]

ALLEEN ALS FRITZ TOCH EEN SPELERNAAM VERPLICHT
----------------------------------------------
Gebruik uitsluitend in dat uitzonderlijke geval:

    Speler          AAN
    Spelernaam       Metal

Dus exact "Metal": hoofdletter M, daarna kleine letters. Gebruik dan niet
"METAL" en niet "metal".

WAAROM DE ANDERE FILTERS OOK UIT BLIJVEN
----------------------------------------
Wit en Zwart blijven uit omdat de Metal-tuning voor beide kleuren moet worden
geleerd.

Overwinningen en Verliespartijen blijven eveneens uit. Metal.pgn bevat een
bewust opgebouwde leerreeks waarmee voorkeuren en ontmoedigingen aan Fritz
worden doorgegeven. Een winst-/verliesfilter kan dat leersignaal onvolledig
maken.

Gebruik daarom altijd de volledige partijenreeks.

Dezelfde Fritz-instellingen gelden ook als Metal.pgn door Ctg2Metal in de
voorzichtige Kleine/Brede-boekmodus is gemaakt.

STAP VOOR STAP
--------------
1. Werk bij voorkeur met een kopie van de complete CTG-set (.ctg/.cto/.ctb).
2. Open die CTG-kopie in Fritz.
3. Kies de Fritz-functie "Leren van database" / "Bijleren uit database".
4. Selecteer de door Ctg2Metal gemaakte Metal.pgn.
5. Zet Overwinningen, Verliespartijen, Wit, Zwart en Speler UIT.
6. Laat het spelernaamveld leeg.
7. Selecteer alle partijen: partij 1 t/m de laatste partij.
8. Start het leerproces en laat het volledig uitlopen.

HOE HERKENT U DAT FRITZ ECHT LEERT?
-----------------------------------
Bij een grote Metal.pgn kan het leerproces duidelijk tijd kosten. Na afloop
kunnen de Fact-/leerwaarden en de daaruit volgende boekpercentages veranderen.

Is een grote Metal.pgn onverwacht vrijwel onmiddellijk klaar en verandert er
niets, controleer dan eerst of alle filters werkelijk UIT staan en of de
volledige partijenreeks is geselecteerd.

KORTE VUISTREGEL
----------------
Voor Metal.pgn in Fritz:

    ALLE FILTERS UIT.
    SPELER NIET AANVINKEN.
    SPELERNAAM LEEG LATEN.
    ALLE PARTIJEN LATEN BIJLEREN.

Alleen als Fritz een spelerfilter verplicht: gebruik exact de naam: "Metal" (hoofdlettergevoelig).
`
...
func run(cfg Config) error {
	...
	if ok > 0 {
		...
		fmt.Println("  Zet Overwinningen, Verliespartijen, Wit, Zwart en Speler UIT.")
...
func processBook(set BookSet, out string, cfg Config) error {
	start := time.Now()
	r := &Report{BookName: set.Base, MaxPly: cfg.MaxPly, Method: "Wilson80 decisive W/L + draw-neutral total score; v2.1 small/broad fallback only with user consent", Policy: strictPolicy, SelectionMode: strictPolicy.Name}
	var err error
	...
	if r.RecordsEmitted == 0 {
		...
		// User explicitly chose the conservative fallback.  Restore the clean
		...
		fmt.Println()
		fmt.Println("3/5 Kleine/Brede-boekmodus gestart op uw verzoek...")
...
func liveLine(s string) {
	// Keep progress short enough to avoid most console wrapping. Pad only what is
	// necessary to erase remnants of the previous live line.
	pad := 0
...
func writeReport(path string, r *Report) error {
	...
	fmt.Fprintf(&sb, "Leerfilter Fritz     : Speler UIT + naamveld leeg; alle partijen; alleen indien verplicht exact de naam: \"Metal\" (hoofdlettergevoelig)\n")
...
func selfTest() error {
	...
	if !strings.Contains(learningInstructions, "Speler               UIT") || !strings.Contains(learningInstructions, "Spelernaam           LEEG") || !strings.Contains(learningInstructions, "ALLE FILTERS UIT") || !strings.Contains(learningInstructions, `exact de naam: "Metal" (hoofdlettergevoelig)`) {
		return errors.New("selftest leerinstructie mist Speler UIT/naamveld leeg of fallback Metal")
	}
