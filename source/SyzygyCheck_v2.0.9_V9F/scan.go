...
func runRealScan(r *bufio.Reader, lang, helper, helperOrigin, dir string, files []item, totalBytes int64, workers int) realScanResult {
	res := realScanResult{Started: time.Now(), Errors: map[string]string{}, EngineMessages: []string{"tbcheck origin: " + helperOrigin}, Workers: workers}
	...
	redrawScanScreen(lang, dir, len(files), workers, eventLines)
...
func redrawScanScreen(lang, dir string, totalFiles, workers int, eventLines []string) {
	clearConsoleScreen()
	drawRunHeader(lang, dir, totalFiles, workers)
