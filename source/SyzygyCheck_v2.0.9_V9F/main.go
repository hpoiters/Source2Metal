...
var texts = map[string]textSet{
	"en": {
		...
		"ERROR: the checksum engine could not be started: %v", "No .rtbw or .rtbz files were found in this folder.", "Press Enter to exit...", "ERROR: tbcheck.exe was not found next to SyzygyCheck. This v2.0.9 build uses the original tbcheck verification engine by Ronald de Man.",
	...
	"de": {
		...
		"FEHLER: Prüfsummen-Engine konnte nicht gestartet werden: %v", "Keine .rtbw- oder .rtbz-Dateien in diesem Ordner gefunden.", "Zum Beenden Enter drücken...", "FEHLER: tbcheck.exe wurde neben SyzygyCheck nicht gefunden. Diese Version v2.0.9 verwendet die ursprüngliche tbcheck-Prüfengine von Ronald de Man.",
	...
	"nl": {
		...
		"FOUT: de checksum-engine kon niet worden gestart: %v", "Geen .rtbw- of .rtbz-bestanden in deze map gevonden.", "Druk op Enter om af te sluiten...", "FOUT: tbcheck.exe staat niet naast SyzygyCheck. Deze v2.0.9-versie gebruikt de oorspronkelijke tbcheck-verificatie-engine van Ronald de Man.",
	...
	"fr": {
		...
		"ERREUR : le moteur de contrôle n'a pas pu démarrer : %v", "Aucun fichier .rtbw ou .rtbz trouvé dans ce dossier.", "Appuyez sur Entrée pour quitter...", "ERREUR : tbcheck.exe est introuvable à côté de SyzygyCheck. Cette version v2.0.9 utilise le moteur de vérification tbcheck original de Ronald de Man.",
	...
	"es": {
		...
		"ERROR: no se pudo iniciar el motor de comprobación: %v", "No se encontraron archivos .rtbw o .rtbz en esta carpeta.", "Pulse Enter para salir...", "ERROR: no se encontró tbcheck.exe junto a SyzygyCheck. Esta versión v2.0.9 usa el motor de verificación tbcheck original de Ronald de Man.",
	...
	"zh": {
		...
		"错误：无法启动校验和引擎：%v", "此文件夹中未找到 .rtbw 或 .rtbz 文件。", "按 Enter 退出...", "错误：SyzygyCheck 旁边未找到 tbcheck.exe。本 v2.0.9 版本使用 Ronald de Man 原始的 tbcheck 验证引擎。",
	...
	"ru": {
		...
		"ОШИБКА: не удалось запустить механизм проверки: %v", "В этой папке не найдены файлы .rtbw или .rtbz.", "Нажмите Enter для выхода...", "ОШИБКА: tbcheck.exe не найден рядом с SyzygyCheck. Эта версия v2.0.9 использует оригинальный движок проверки tbcheck Ronald de Man.",
...
func runCheck(r *bufio.Reader, lang string) bool {
	...
	if !testResult.ExpectedFailDetected {
		reportPath, reportErr := writeSelfTestFailureReportV6(lang, dir, helperHandle.Origin, testResult)
		if reportErr == nil {
			fmt.Println()
	...
	// parent folders and no subfolders.
	files, totalBytes, err := discoverFiles(dir)
	if err != nil {
		fmt.Printf("\n"+t.engineStart+"\n", err)
		waitEnter(r, t.pressEnter)
		return true
	...
	scan := runRealScan(r, lang, helperHandle.Path, helperHandle.Origin, dir, files, totalBytes, workers)
	if scan.Cancelled {
		return false
	}
	...
	reportPath, reportErr := writeReportV6(lang, dir, files, totalBytes, scan, testResult)
...
func discoverFiles(dir string) ([]item, int64, error) {
	...
	for _, e := range entries {
		...
		files = append(files, item{name: e.Name(), size: info.Size()})
		total += info.Size()
	}
...
func drawRunHeader(lang, dir string, n, workers int) {
	t := texts[lang]
	fmt.Printf("SyzygyCheck v%s - %s\n", version, t.all)
	fmt.Printf("%s: %s\n", t.language, displayFor(lang))
	printWrappedStatic(t.credit)
