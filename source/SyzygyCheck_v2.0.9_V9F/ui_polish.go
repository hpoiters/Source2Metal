...
func reportSavedScreenLines(lang, path string) []string {
	...
	switch lang {
	case "nl":
		return []string{"Resultaatrapport kon niet op uw Windows-bureaublad worden opgeslagen; het staat daarom naast SyzygyCheck.", "Volledig pad: " + path}
	case "de":
		return []string{"Der Ergebnisbericht konnte nicht auf dem Windows-Desktop gespeichert werden und wurde daher neben SyzygyCheck abgelegt.", "Vollständiger Pfad: " + path}
	case "fr":
		return []string{"Le rapport n'a pas pu être enregistré sur le Bureau Windows ; il a donc été placé à côté de SyzygyCheck.", "Chemin complet : " + path}
	case "es":
		return []string{"El informe no pudo guardarse en el Escritorio de Windows; por ello se guardó junto a SyzygyCheck.", "Ruta completa: " + path}
	case "zh":
		return []string{"结果报告无法保存到 Windows 桌面，因此已保存在 SyzygyCheck 旁边。", "完整路径：" + path}
	case "ru":
		return []string{"Отчёт не удалось сохранить на рабочем столе Windows, поэтому он сохранён рядом с SyzygyCheck.", "Полный путь: " + path}
	default:
		return []string{"The result report could not be saved on the Windows desktop, so it was saved next to SyzygyCheck.", "Full path: " + path}
	}
}
