...
func buildRawBooks(inv Inventory, layout OutputLayout, cfg Config, c *Counters) ([]string, error) {
	...
	for i, s := range sets {
		...
		res, err := ctgraw.BuildGraph(s.Path, s.Aux[0], s.Aux[1], tmpOut, cfg.CTGMaxPly, cfg.Workers, func(text string, final bool) {
			if final {
		...
		for _, suffix := range []string{".ctg-report.json", ".ctg-statistics.jsonl"} {
			if err := os.Rename(tmpOut+suffix, finalOut+suffix); err != nil {
				return outputs, err
			}
		}
...
func buildMetalFromCTGLegacy(inv Inventory, layout OutputLayout, cfg Config, c *Counters) error {
	sets := completeCTGSources(inv)
	if len(sets) == 0 {
...
// CTG-METAL now reuses the neutral RAW coverage instead of sampling or
// manufacturing win/loss learning signals. GAME/BIN processing is unchanged.
func buildMetalFromCTG(inv Inventory, layout OutputLayout, cfg Config, c *Counters) error {
	for _, s := range completeCTGSources(inv) {
		tr := getSourceTrace(c, KindCTG, s.Base, s.Path)
		if tr.RawAccepted == 0 {
			c.CTGMetalSkipped++
			continue
		}
		dir := uniqueSourceDir(layout.MetalSeparateDir, s.Base, KindCTG, s.Path)
		if e := os.MkdirAll(dir, 0755); e != nil {
			return e
		}
		path := filepath.Join(dir, fmt.Sprintf("%s [CTG] - METAL.pgn", safeComponent(s.Base)))
		in, e := os.Open(tr.RawPath)
		if e != nil {
			return e
		}
		...
		tr.MetalPath = path
		tr.MetalSelected = tr.RawAccepted
		c.CTGMetalBuilt++
		c.CTGMetalBytes += n
	}
	return nil
...
func mergeFinalMetal(layout OutputLayout, c *Counters) error {
	paths := []string{}
	if c.GameMetalSelected > 0 {
		paths = append(paths, layout.GameMetalFile)
	}
	for _, tr := range c.SourceTraces {
		if tr.Kind == KindCTG && tr.MetalSelected > 0 {
			paths = append(paths, tr.MetalPath)
		}
	}
	if len(paths) == 0 {
		return nil
	}
	if e := os.MkdirAll(layout.MetalDir, 0755); e != nil {
		return e
	}
	final := filepath.Join(layout.MetalDir, "Metal.pgn")
	out, e := os.Create(final)
	if e != nil {
		return e
	}
	var expected int64 = c.GameMetalSelected
	for _, tr := range c.SourceTraces {
		if tr.Kind == KindCTG {
			expected += tr.MetalSelected
		}
	}
	for _, path := range paths {
		e = parsePGNFile(path, func(g PGNGame) error {
			result := g.Tags["Result"]
			if result != "*" && result != "1-0" && result != "0-1" && result != "1/2-1/2" {
				return fmt.Errorf("invalid final METAL result")
			}
			_, e := fmt.Fprintf(out, "[Event \"Source2Metal neutral training set\"]\n[Site \"?\"]\n[Date \"????.??.??\"]\n[Round \"?\"]\n[White \"Metal\"]\n[Black \"Metal\"]\n[Result \"%s\"]\n\n%s\n\n", result, g.MoveText)
			return e
		}, nil)
		if e != nil {
			out.Close()
	...
	found, e := countGeneratedPGNRecords(final)
	if e != nil {
		return e
	}
	if found != expected {
		return fmt.Errorf("merged Metal records %d != %d", found, expected)
	}
	return nil
}
