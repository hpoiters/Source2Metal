...
func parseConfig() (Config, error) {
	...
	flag.IntVar(&c.CTGMaxPly, "ctg-max-ply", -1, "CTG depth: -1=RAW depth, 0=unlimited graph, positive=half-moves")
	...
	if c.CTGMaxPly < -1 {
		return c, fmt.Errorf("ctg-max-ply must be -1 or greater")
	}
	return c, nil
}
...
func run(cfg Config) error {
	...
	if cfg.Mode == "all" {
		if err := mergeFinalMetal(layout, &c); err != nil {
			return err
		}
	}
	...
	fmt.Println("SOURCE: Source2Metal_v3.3.0_SOURCE.zip")
	return nil
}
...
func classifyRunStatus(cfg Config, c Counters) string {
	...
	if c.CTGDecodeErrors > 0 || c.CBHSetsFailed > 0 || c.TwoCBHSetsFailed > 0 || c.CTGRawFailed > 0 || c.CTGMetalFailed > 0 || c.GameMetalFailed > 0 {
...
func interactiveChoices(c Config, inv Inventory, hw HardwareInfo) (Config, error) {
	...
	if c.Mode != "inventory" {
		...
		c.CTGMaxPly = askInt("CTG depth / diepte [0=unlimited/onbegrensd, Enter=RAW]: ", c.MaxPly, 0, 1000000)
