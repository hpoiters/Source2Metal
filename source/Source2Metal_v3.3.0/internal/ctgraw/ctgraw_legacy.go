...
func (r *CTGReader) pageIndices(sig []byte) []int32 {
	h := int32(ctgHash(sig))
	low, high := int32(r.low), int32(r.high)
	out := make([]int32, 0, 32)
	// Match the robust Ctg2Metal/DroidFish probing rule: CTB upper is a stop
	// threshold, not a hard CTO index cap. The first key that reaches or
	// overshoots upper must still be probed.
	for mask := int32(0); mask < 0x7fffffff; mask = 2*mask + 1 {
		key := (h & mask) + mask
		if key < low {
			continue
		}
...
var errPositionMissing = errors.New("positie niet gevonden via geprobeerde CTG-pagina's")

func (r *CTGReader) Lookup(b Board) (Entry, error) {
	sig := b.Signature()
	indices := r.pageIndices(sig)
	if len(indices) == 0 {
		return Entry{}, errors.New("positie niet in CTO-index")
	}
	for _, key := range indices {
		var ib [4]byte
		off := int64(16) + int64(key)*4
		if _, e := r.cto.ReadAt(ib[:], off); e != nil {
			return Entry{}, fmt.Errorf("CTO index %d buiten/ongeldig (offset %d): %w", key, off, e)
		}
		idx := int32(binary.BigEndian.Uint32(ib[:]))
		if idx < 0 {
			continue
		}
		pg, e := r.page(idx)
		...
		for i := 0; i < n; i++ {
			if pos >= len(pg) {
				return Entry{}, fmt.Errorf("truncated CTG page %d entry %d", idx, i)
			}
			sl := int(pg[pos] & 31)
			if sl <= 0 || pos+sl >= len(pg) {
				return Entry{}, fmt.Errorf("invalid CTG signature page %d entry %d", idx, i)
			}
			moveSize := int(pg[pos+sl])
			if moveSize < 1 || moveSize%2 == 0 || pos+sl+moveSize+33 > len(pg) {
				return Entry{}, fmt.Errorf("invalid CTG record page %d entry %d", idx, i)
			}
			if sl == len(sig) && string(pg[pos:pos+sl]) == string(sig) {
				...
				q := pos + sl + moveSize
				en := Entry{Moves: moves, RawFooter: append([]byte(nil), pg[q:q+33]...)}
	...
	return Entry{}, errPositionMissing
}
...
func (p *Progress) Update(done int, force bool) {
	...
	line := fmt.Sprintf("Maken: [%-20s] %3.0f%% | resterend ca. %s", bar, frac*100, txt)
	if integratedProgress != nil {
		integratedProgress(line, force)
		return
	}
	fmt.Printf("\r%s", line)
	if force {
...
func targetGames(root Entry) int {
	...
	// RAW output follows the actually decodable CTG source content; no synthetic minimum floor.
	// should reflect the evidence actually present in the CTG instead of
	// inflating a small book into a large synthetic database.
	if n < 1 {
		n = 1
	}
	if n > MaxTargetGames {
...
func writeNeutralPGN(reader *CTGReader, outPath, bookName string, maxPly int) (games int, plies int64, err error) {
	...
	if total < len(rootChoices) {
		total = len(rootChoices)
	}
