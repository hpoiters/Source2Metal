...
In source/Source2Metal_v3.2.1, run prepare_syzygy_package.ps1 once.
...
duplicate BIN books, GAME-filter exemption, malformed BIN rejection, saved
interactive language selection and the v3.2.1 PGN-resilience regression. The
regression verifies that unusable games do not abort the batch, Event-less
source games do not cause a false RAW verification failure, and a following PGN
source is still processed. It publishes v3.2.1 as Latest only on main, after all
checks succeed. Existing tags and release assets are never rewritten.
...
and unchanged separate GAME-METAL/CTG-METAL routes. Automated Windows tests use
synthetic inputs. Optional external CTG RAW integration fixtures can be provided
through SOURCE2METAL_CTG_RAW_TEST_DIR when running go test.
